...
public enum GameMenuCommands {
    DROP_BUILDING_REGEX("(?=.* -x (?<x>\\S+))(?=.* -y (?<y>\\S+))(?=.* -type (?<type>.+))^dropbuilding( *-[xytpe]+.+){3}$"),
    ...
    PATROL_UNIT_REGEX("(?=.* -x1 (?<x1>\\d+))(?=.* -y1 (?<y1>\\d+))(?=.* -x2 (?<x2>\\S+))(?=.* -y2 (?<y2>\\d+))^patrol unit( *-[xy12]+ \\d+){4}$"),
    ...
    SET_UNIT_MODE_REGEX("(?=.* -x (?<x>\\S+))(?=.* -y (?<y>\\S+))(?=.* -s (?<mode>\\S+))^set ( *-[xys] \\S+){3}$");
