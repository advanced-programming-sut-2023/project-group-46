...
//                , signupMenu.getNickname().getText(), signupMenu.getSlogan().getText(), "users.json", "/Image/Avatar/10.png");
        ...
        String password = signupMenu.getPassword().getText();
        ...
        User user1 = new User(signupMenu.getUsername().getText() , hashedPassword , signupMenu.getNickname().getText() , signupMenu.getEmail().getText() ,signupMenu.getPasswordRecovery().getText());

        user1.friendsOfUser = new ArrayList<>();
        ArrayList<User> add = User.getAllUsersMain();
        add.add(user1);

        Gson gson = new GsonBuilder().setPrettyPrinting().create();

        try (FileWriter writer = new FileWriter("users.json")) {
            gson.toJson(add, writer);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//        Files.write(Paths.get("users.json"), usersArray.toString().getBytes());
        ...
        Database.chats.get(0).usernames.add(signupMenu.getUsername().getText());
    ...
    public void writeInJsonFile(String username, String password, String email, String nickName, String slogan, String fileName, String image) throws IOException, IOException {
        ...
        newUserMap.put("last seen time" , "");
        newUserMap.put("online" , "false");
        newUserMap.put("friendRequests" , new ArrayList<String>());
        newUserMap.put("maps" , new ArrayList<String>());
