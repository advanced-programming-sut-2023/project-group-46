...
public class ShopMenuController {
    ...
    public String buy(Matcher matcher) {
        ...
        if (GameMenuController.getCurrentEmpire().getResources().isResourceType(name) && GameMenuController.getCurrentEmpire().getBuildingByName("stockpile").getFreeCapacity() < amount) {
            return "not enough space in the stockpile";
        } else if (GameMenuController.getCurrentEmpire().getArmoury().isArmouryType(name) && GameMenuController.getCurrentEmpire().getBuildingByName("armoury").getFreeCapacity() < amount) {
            return "not enough space in the armoury";
        } else if (GameMenuController.getCurrentEmpire().getFoodStock().isFoodType(name) && GameMenuController.getCurrentEmpire().getBuildingByName("foodstock").getFreeCapacity() < amount) {
            return "not enough space in the food stock";
        }
    ...
    public String sell(Matcher matcher) {
        ...
        if (GameMenuController.getCurrentEmpire().getResources().isResourceType(name) && GameMenuController.getCurrentEmpire().getResources().getResourceAmount(name) < amount) {
            ...
            return "not enough food in the food stock";
        }
